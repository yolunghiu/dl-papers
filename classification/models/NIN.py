...
def model():
    ...
    with tf.name_scope('data'):
        x = tf.placeholder(tf.float32, shape=[
                           None, _IMAGE_SIZE * _IMAGE_SIZE * _IMAGE_CHANNELS], name='Input')
        y = tf.placeholder(tf.float32, shape=[
                           None, _NUM_CLASSES], name='Output')
        x_image = tf.reshape(
            x, [-1, _IMAGE_SIZE, _IMAGE_SIZE, _IMAGE_CHANNELS], name='images')
    ...
    x = tf.placeholder(
        tf.float32, [None, _IMAGE_SIZE * _IMAGE_SIZE * _IMAGE_CHANNELS])
    ...
    with tf.name_scope('conv1'):
        ...
        b_conv1 = tf.Variable(tf.random_normal(
            [192], stddev=0.01, dtype=tf.float32))
        output = tf.nn.relu(conv2d(x_image, W_conv1) + b_conv1)
        print('conv1 output: ', output.shape)   # [None, 32, 32, 3]
        ...
        with tf.name_scope('mlp_1_1'):
            ...
            print('MLP-1-1 output: ', output.shape)     # [None, 32, 32, 160]
        ...
        with tf.name_scope('mlp_1_2'):
            ...
            print('MLP-1-2 output: ', output.shape)     # [None, 32, 32, 96]
            tf.summary.histogram('mlp', output)
            tf.summary.scalar('mlp', tf.nn.zero_fraction(output))
    ...
    with tf.name_scope('conv2'):
        ...
        b_conv2 = tf.Variable(tf.random_normal(
            [192], stddev=0.01, dtype=tf.float32))
    ...
    with tf.name_scope('conv3'):
        ...
        b_conv3 = tf.Variable(tf.random_normal(
            [192], stddev=0.01, dtype=tf.float32))
        ...
        with tf.name_scope('mlp_3_2'):
            ...
            # [None, 8, 8, 10]
            print('Before global average: ', output.shape)
            tf.summary.histogram('mlp', output)
            tf.summary.scalar('mlp', tf.nn.zero_fraction(output))
        # global average
        output = tf.nn.avg_pool(output, ksize=[1, 8, 8, 1], strides=[
                                1, 1, 1, 1], padding='VALID')
        print('After global average: ', output.shape)   # [None, 1, 1, 10]
